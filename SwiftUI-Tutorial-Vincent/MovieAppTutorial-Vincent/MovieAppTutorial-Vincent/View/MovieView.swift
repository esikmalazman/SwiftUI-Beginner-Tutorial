...
struct MovieView: View {
    ...
    var body: some View {
        /// `Group` :  allow to collect view as container without impacting the layout and it is a logical container and it still allow to call a modifer like .task modifier
        Group {
            /// SwitUI allow to iterate the enum case by using switch conditional statement in the body
            switch viewModel.state {
                /// show the  progress view when the viewModel state loading
            case .loading :
                ProgressView()
            case .error(let error) :
                /// show the error in text view when the viewModel state error
    ...
    /// refactor the code to build `List View` in its own dedicated method
    /// `some` : opaque return type, it way to tell compiler that the return result would be something that adopt View protocol
    func list(of movies : [Movie]) -> some View {
        ...
        List(movies) { movie in /// In the closure, it will give each element of the list that allow us to built in the row
